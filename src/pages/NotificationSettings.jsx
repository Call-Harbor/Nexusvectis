...
const PRESETS = [
  {
    name: "Kritisk brændstofniveau",
    entity_type: "Vehicle",
    event_type: "threshold",
    severity: "warning",
    conditions: [{ field: "fuel_level", operator: "less_than", value: "20" }],
    channels: { in_app: true, email: false },
    message_template: "Køretøj {{name}} har lavt brændstofniveau: {{fuel_level}}%. Destination: {{destination}}",
    cooldown_minutes: 120,
  },
  {
    name: "Forsendelse forsinket",
    entity_type: "Shipment",
    event_type: "status_change",
    severity: "warning",
    conditions: [{ field: "status", operator: "equals", value: "delayed" }],
    channels: { in_app: true, email: false },
    message_template: "Forsendelse {{tracking_number}} ({{origin}} → {{destination}}) er forsinket. Kunde: {{customer_name}}",
    cooldown_minutes: 60,
  },
  {
    name: "Kritisk vedligehold",
    entity_type: "Maintenance",
    event_type: "create",
    severity: "critical",
    conditions: [{ field: "priority", operator: "equals", value: "critical" }],
    channels: { in_app: true, email: false },
    message_template: "Kritisk vedligehold oprettet: {{component}} — {{description}}",
    cooldown_minutes: 0,
  },
  {
    name: "Køretøj offline",
    entity_type: "Vehicle",
    event_type: "status_change",
    severity: "critical",
    conditions: [{ field: "status", operator: "equals", value: "offline" }],
    channels: { in_app: true, email: false },
    message_template: "Køretøj {{name}} er gået offline. Sidst set: {{updated_date}}",
    cooldown_minutes: 30,
  },
];
