            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="bg-slate-900 border-slate-800 max-h-96 overflow-y-auto">
                  ...
                  {[['nexus_chat', MessageSquare, 'Chat'], ['document_editor', FileText, 'Document Editor'], ['spreadsheet_editor', BarChart3, 'Spreadsheet']].map(([type, Icon, label]) => (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="bg-slate-900 border-slate-800 max-h-96 overflow-y-auto w-80">
                  {[
                    ['Fleet Operations', [['Vehicle Efficiency Analysis', "Analyser alle køretøjer og identificer dem med laveste effektivitetsscore."], ['Route Optimization', "Optimer alle aktive ruter baseret på aktuel trafik og vejrforhold."], ['Shipment Risk Assessment', "Gennemgå alle forsendelser i transit og identificer risiko for forsinket levering."]]],
                    ['Predictive Analytics', [['Predictive Maintenance', "Forudsig vedligeholdelsesbehov for alle køretøjer i næste 30 dage."], ['Demand Forecasting', "Lav 90-dages efterspørgselsprognose baseret på historiske data."], ['ETA Accuracy Analysis', "Analyser ETA-nøjagtighed over sidste 60 dage."]]],
                    ['Risk & Compliance', [['Risk Assessment', "Udfør omfattende risikovurdering af hele flåden."], ['Safety Analysis', "Analyser alle sikkerheds- og sikkerhedsvarsler fra sidste 90 dage."], ['Compliance Check', "Kontroller compliance med alle gældende transportbestemmelser."]]],
                    ['Financial & Performance', [['Cost Analysis', "Analyser transportomkostninger per km, per enhed, per rute."], ['Sustainability Report', "Beregn CO2-emissioner for alle forsendelser."], ['Driver Performance', "Evaluer præstationen for hver chauffør."]]],
                    ['Strategic Planning', [['Capacity Planning', "Analyser flådekapacitet mod efterspørgsel."], ['Root Cause Analysis', "Identificer mønstre i forsinkelser, afvisninger og fejl."]]],
                  ].map(([section, items]) => (
                  ...
                  <DropdownMenuItem onClick={() => openWindow('deep_analysis', { x: 100, y: 80 })} className="text-slate-300 text-xs"><Activity className="w-4 h-4 mr-2 text-cyan-400" />Anomaly Detection + What-If + Proactive AI</DropdownMenuItem>
        ...
        <div className="p-3 sm:p-4 lg:p-6">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-3 sm:mb-4 max-h-32 sm:max-h-48 overflow-y-auto space-y-1.5 sm:space-y-2">
              {messages.slice(-5).map((msg, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-1">
                  <div className={`text-sm p-3 rounded-lg backdrop-blur-xl ${msg.role === 'user' ? 'bg-cyan-500/10 border border-cyan-500/20 text-cyan-300' : msg.role === 'system' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-300' : 'bg-slate-800/50 border border-slate-700/50 text-slate-200'}`}>
                    <span className="font-semibold mr-1">{msg.role === 'user' ? '>' : msg.role === 'system' ? '⚡' : '🧠'}</span>
                    {msg.streaming ? <span className="animate-pulse">{msg.content || 'Thinking...'}</span> : msg.role === 'assistant' ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 sm:gap-3">
                <input type="text" value={input}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); processCommand(); }
                  ...
                  placeholder="Command FLEET AI... (e.g. 'predict maintenance', 'forecast demand', 'analyze CO2 emissions')"
                ...
                <Button onClick={async () => {
                  ...
                  recognition.start();
                }} size="sm" className={`px-3 sm:px-4 lg:px-6 ${isListening ? 'bg-red-500 hover:bg-red-600' : 'bg-slate-800 hover:bg-slate-700'} rounded-xl sm:rounded-2xl hidden sm:flex`}>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
              <div className="flex items-center gap-2"><Zap className="w-3 h-3 text-cyan-500" /><span>Press Enter to send • ↑↓ for history</span></div>
