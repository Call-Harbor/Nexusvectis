...
export default function IntellectMode() {
  ...
  const handleQuickAction = useCallback((action) => {
   const actionMap = {
     ...
     openHarborAppBuilder: () => { openWindow('harbor_app_builder', { x: 60, y: 50 }, { installedAppIds, onInstall: (appId) => setInstalledAppIds(prev => new Set([...prev, appId])), vehicles, routes, shipments, alerts, customers, currentUser, orgId }); setMessages(prev => [...prev, { role: "system", content: "⚡ H.A.R.B.O.R App Builder activated — Entity-first design workflow" }]); },
     openFleetStore: () => { openWindow('fleet_store', { x: 100, y: 80 }, { installedAppIds, onInstall: (appId) => { setInstalledAppIds(prev => new Set([...prev, appId])); openWindow('harbor_app_builder', { x: 60, y: 50 }, { installedAppIds: new Set([...installedAppIds, appId]), onInstall: (id) => setInstalledAppIds(prev => new Set([...prev, id])), vehicles, routes, shipments, alerts, customers, currentUser, orgId, autoInstallAppId: appId }); toast.success("✅ App tilføjet — åbner i App Builder!"); }, onUninstall: async (appId) => { setInstalledAppIds(prev => { const next = new Set(prev); next.delete(appId); return next; }); try { const copies = await base44.entities.HarborApp.filter({ organization_id: orgId }); const copy = copies.find(a => a.prompt && a.id !== appId); if (copy) await base44.entities.HarborApp.delete(copy.id); } catch {} } }); setMessages(prev => [...prev, { role: "system", content: "🏪 Fleet Store opened — Browse and install published apps" }]); },
     };
     actionMap[action]?.();
