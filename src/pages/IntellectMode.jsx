...
export default function IntellectMode() {
  ...
  const handleQuickAction = useCallback((action) => {
   const actionMap = {
     ...
     openFleetStore: () => { openWindow('fleet_store', { x: 100, y: 80 }, { installedAppIds, onInstall: (appId) => { setInstalledAppIds(prev => new Set([...prev, appId])); toast.success("App installed!"); } }); setMessages(prev => [...prev, { role: "system", content: "🏪 Fleet Store opened — Browse and install published apps" }]); },
     };
     actionMap[action]?.();
