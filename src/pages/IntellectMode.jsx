...
export default function IntellectMode() {
  ...
  const handleQuickAction = useCallback((action) => {
   const actionMap = {
     ...
     openFleetStore: () => { openWindow('fleet_store', { x: 100, y: 80 }, { installedAppIds, onInstall: (appId) => { setInstalledAppIds(prev => new Set([...prev, appId])); // Dispatch event so any open App Builder window picks it up window.dispatchEvent(new CustomEvent('harbor_install_app', { detail: { appId, orgId } })); // Also ensure App Builder is open openWindow('harbor_app_builder', { x: 60, y: 50 }, { installedAppIds: new Set([...installedAppIds, appId]), onInstall: (id) => setInstalledAppIds(prev => new Set([...prev, id])), vehicles, routes, shipments, alerts, customers, currentUser, orgId }); toast.success("✅ App installeret — tjek App Builder!"); }, onUninstall: async (appId) => { setInstalledAppIds(prev => { const next = new Set(prev); next.delete(appId); return next; }); } }); setMessages(prev => [...prev, { role: "system", content: "🏪 Fleet Store opened — Browse and install published apps" }]); },
     };
     actionMap[action]?.();
