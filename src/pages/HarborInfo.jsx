...
const INTEGRATIONS = [
  { icon: Network, label: "IntellectMode", desc: "Primary AI command interface. HARBOR routes all commands through harborCore, determines action type (ANSWER / OPEN_WINDOW / CREATE_DOCUMENT / CREATE_SPREADSHEET / SHOW_ANALYSIS / SHOW_3D), and orchestrates hologram window rendering.", color: "text-cyan-400", badge: "Live" },
  { icon: Radar, label: "Fleet AI API (harborModelInference)", desc: "External developer API via Bearer token. HARBOR validates API keys by SHA-256 hash match, loads the requested FleetAIModel snapshot, routes inference through harborCore, and logs every call to APIUsage.", color: "text-violet-400", badge: "REST API" },
  { icon: FlaskConical, label: "HARBOR Trainer", desc: "Knowledge injection UI. Operators upload FAQs, links, SOPs, and training files. On Save Model, data is persisted as a FleetAIModel entity and automatically injected into every future HARBOR query for that org.", color: "text-emerald-400", badge: "ML Pipeline" },
  { icon: Eye, label: "mistralCommand / harborCore", desc: "All IntellectMode commands go through mistralCommand → harborCore. harborCore loads the org's active FleetAIModel training data, injects live context, calls Mistral Large, self-heals malformed JSON, and returns a structured action.", color: "text-amber-400", badge: "Core Engine" },
  { icon: GitBranch, label: "Entity Actions", desc: "HARBOR executes 15+ platform operations directly: CREATE_VEHICLE, CREATE_ROUTE, CREATE_SHIPMENT, CREATE_ALERT, UPDATE_VEHICLES, UPDATE_ROUTES, DELETE_VEHICLES, DELETE_ROUTES, and more — all triggered by parsed AI intent.", color: "text-orange-400", badge: "Autonomous" },
  { icon: Workflow, label: "Parallel Micro-Analyses", desc: "Every command also fires 49 parallel lightweight LLM micro-analyses (efficiency, anomaly, route time, ETA, maintenance risk, cost saving, safety, demand) simultaneously alongside the main HARBOR response.", color: "text-rose-400", badge: "Parallel" },
];
