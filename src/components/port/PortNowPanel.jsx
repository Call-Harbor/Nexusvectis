    ...
    <div className="w-72 flex-shrink-0 space-y-3">
      ...
      <div className="rounded-2xl p-4" style={{ background: "rgba(0,8,20,0.85)", border: "1px solid rgba(6,182,212,0.2)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[9px] font-black tracking-[0.3em] uppercase text-cyan-400">AKTIVE OPERATIONER</p>
          <span className="ml-auto text-[9px] font-black text-cyan-300">{active.length}</span>
        </div>
        {active.length === 0 && (
          <p className="text-[10px] text-slate-600 text-center py-3">Ingen aktive operationer</p>
        )}
        {active.map(pc => {
      ...
      <div className="rounded-2xl p-4" style={{ background: "rgba(0,8,20,0.85)", border: "1px solid rgba(51,65,85,0.3)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[9px] font-black tracking-[0.3em] uppercase text-amber-400">KRANER</p>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "TOTAL", val: cranes.length, color: "#06b6d4" },
            { label: "AKTIV", val: cranesWorking, color: "#10b981" },
            { label: "FEJL", val: cranesBroken, color: cranesBroken > 0 ? "#f43f5e" : "#334155" },
          ].map(k => (
      ...
      <div className="rounded-2xl p-4" style={{ background: "rgba(0,8,20,0.85)", border: "1px solid rgba(51,65,85,0.3)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[9px] font-black tracking-[0.3em] uppercase text-violet-400">YARD ZONER</p>
        </div>
        <div className="space-y-2">
          ...
          {yardZones.length === 0 && <p className="text-[10px] text-slate-600 text-center py-2">Ingen yard zoner</p>}
        </div>
        {yardZones.length > 0 && (
          <div className="mt-3 pt-2 border-t border-slate-800/60 flex items-center justify-between">
            <span className="text-[8px] text-slate-600 uppercase tracking-widest">GNS BELASTNING</span>
      ...
      <div className="rounded-2xl p-4" style={{ background: "rgba(0,8,20,0.85)", border: "1px solid rgba(51,65,85,0.3)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[9px] font-black tracking-[0.3em] uppercase text-slate-400">KOMMENDE ANLØB</p>
        </div>
        {upcoming.length === 0 && <p className="text-[10px] text-slate-600 text-center py-2">Ingen planlagte anløb</p>}
        {upcoming.map(pc => (
