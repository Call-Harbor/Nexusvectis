...
import { PredictiveMaintenanceAnalysis, DemandForecastAnalysis, RiskAssessmentAnalysis, PerformanceAnalyticsPanel } from "@/components/intellect/AdvancedAIAnalysis";
...
export default function WindowContentRenderer({ type, data, vehicles, routes, shipments, alerts, currentUser, orgId, customers, setInput, openWindow }) {
  ...
  if (type === 'company_analytics' || type === 'company_analysis') return <CompanyAnalysisHologram onClose={data?.onClose} onSendToScreen={null} embedded={true} />;
  ...
  if (type === 'route_optimization' || type === 'route_optimizer') return <RealTimeRouteOptimizationPanel orgId={orgId} optimizeFor={data?.optimizeFor || 'time'} />;
  if (type === 'demand_forecast') return <DemandForecastAnalysis vehicles={vehicles} routes={routes} shipments={shipments} />;
