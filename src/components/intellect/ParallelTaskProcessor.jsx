...
import {
  Zap, Loader2, CheckCircle2, AlertCircle, X, Trash2, Play, Pause,
...
export default function ParallelTaskProcessor({ onClose }) {
  ...
  useEffect(() => {
    if (isPaused || runningTasks.length >= MAX_PARALLEL || queuedTasks.length === 0) {
      return;
    }
  ...
  }, [runningTasks.length, queuedTasks, isPaused]);

  const executeTask = async (task) => {
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Perform this analysis:\n\n${task.prompt}`,
  ...
  const addPrompt = () => {
    ...
    setInputValue("");
    toast.success("Prompt tilføjet til kø");
  };

  const addMultiplePrompts = (text) => {
    ...
    setInputValue("");
    toast.success(`${lines.length} prompts tilføjet til kø`);
  };
      ...
      <div className="p-4 border-b border-slate-800 flex-shrink-0">
        ...
        <div className="space-y-2">
          <textarea
            ...
            placeholder="Enter prompt here... or paste multiple lines for batch processing"
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => addMultiplePrompts(inputValue)}
            ...
            <Button
              ...
              className={isPaused ? "bg-amber-600 hover:bg-amber-700" : "bg-emerald-600 hover:bg-emerald-700"}
              size="sm"
            >
          ...
          <div className="p-4 border-b border-slate-800/50">
            ...
            <div className="space-y-2">
              {runningTasks.map(task => (
                ...
                >
                  <p className="text-white text-xs mb-2 line-clamp-2">{task.prompt}</p>
                  <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              {queuedTasks.map((task, idx) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg flex items-start justify-between gap-2"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-purple-300 text-[10px] font-semibold mb-1">#{idx + 1} in queue</p>
                    <p className="text-white text-xs line-clamp-2">{task.prompt}</p>
                  </div>
                  <Button
                    size="icon"
                    variant="ghost"
              ...
              {completedTasks.map(task => (
                ...
                >
                  <div className="flex items-start gap-2 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-xs line-clamp-2 mb-1">{task.prompt}</p>
        ...
        {totalTasks === 0 && (
          <div className="flex-1 flex items-center justify-center text-slate-500">
            <div className="text-center">
              ...
              <p className="text-sm">Add prompts to get started</p>
            </div>
          </div>
        )}
      </div>
