...
export default function AIDevOrchestrator({ onClose }) {
  ...
  // ── Nexus Cloud: Save project ───────────────────────────────────────────────
  const saveToCloud = async () => {
    if (isSaving) return;
    setIsSaving(true);
    log('$ nexus cloud save', 'system');
    try {
      ...
      await base44.entities.FleetDriveFile.create({
        ...
        file_url,
        folder: 'ide_projects',
        description: `${files.length} files · Saved from Fleet AI IDE`,
        source: 'document_editor',
        tags: ['ide', 'project'],
      });
      log(`✓ Project saved to Nexus Cloud: ${projectData.projectName}`, 'success');
      toast.success('Project saved to Nexus Cloud!');
    } catch (e) {
  ...
  // ── Nexus Cloud: Browse & open projects ─────────────────────────────────────
  const openProjectBrowser = async () => {
    setShowProjectBrowser(true);
    setIsLoadingProjects(true);
    try {
      const user = await base44.auth.me();
      const projects = await base44.entities.FleetDriveFile.filter({
  ...
  const loadCloudProject = async (project) => {
    try {
      ...
      if (data.files?.length) {
        ...
        log(`✓ Loaded project: ${project.name}`, 'success');
        toast.success(`Opened: ${project.name}`);
      }
    } catch (e) {
      toast.error('Could not load project: ' + e.message);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-white overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-slate-800 bg-slate-900 flex-shrink-0 flex-wrap">
        ...
        <button onClick={saveToCloud} disabled={isSaving} className="flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium bg-emerald-700 hover:bg-emerald-600 disabled:opacity-50 text-white transition-all" title="Save project to Nexus Cloud">
          {isSaving ? <Loader2 className="w-3 h-3 animate-spin" /> : <CloudUpload className="w-3 h-3" />}<span className="hidden sm:inline">Save</span>
        </button>
        ...
        {showProjectBrowser && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              className="bg-slate-900 border border-emerald-500/40 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
              <div className="flex items-center gap-2 mb-4">
                <FolderGit2 className="w-5 h-5 text-emerald-400" />
                <h2 className="text-lg font-bold text-white">Nexus Cloud Projects</h2>
                <button onClick={() => setShowProjectBrowser(false)} className="ml-auto text-slate-500 hover:text-white"><X className="w-4 h-4" /></button>
              </div>
              {isLoadingProjects ? (
                ...
                <div className="text-center py-10">
                  <CloudUpload className="w-10 h-10 text-slate-700 mx-auto mb-3" />
                  <p className="text-slate-400 text-sm">No saved projects yet</p>
                  <p className="text-slate-600 text-xs mt-1">Click "Save" to save your current project to Nexus Cloud</p>
                </div>
              ) : (
                <div className="space-y-2 max-h-80 overflow-auto">
                  {cloudProjects.map(proj => (
                    <button key={proj.id} onClick={() => loadCloudProject(proj)}
                      className="w-full flex items-start gap-3 p-3 rounded-xl border border-slate-700 hover:border-emerald-500/50 hover:bg-emerald-500/5 text-left transition-all group">
                      <FolderGit2 className="w-5 h-5 text-emerald-400 flex-shrink-0 mt-0.5" />
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-medium truncate">{proj.name}</p>
                        <p className="text-slate-500 text-xs mt-0.5">{proj.description}</p>
                        <p className="text-slate-600 text-[10px] mt-1">{new Date(proj.created_date).toLocaleDateString('da-DK', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                      <FolderOpen className="w-4 h-4 text-slate-600 group-hover:text-emerald-400 flex-shrink-0 mt-1 transition-colors" />
                    </button>
                  ))}
                </div>
              )}
              <button onClick={() => setShowProjectBrowser(false)} className="mt-4 w-full py-2 rounded-xl border border-slate-700 text-slate-400 hover:text-white text-sm transition-all">Close</button>
            </motion.div>
          </motion.div>
        )}
