...
export default function AISpreadsheetEditor({ initialGrid, initialTitle, initialFileUrl, orgId, onSaved }) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950/80 select-none">
      ...
      {/* ── Header ── JARVIS style */}
      <div className="flex-shrink-0 relative px-3 py-2.5 flex items-center gap-2"
        style={{ background: "rgba(0,10,25,0.97)", borderBottom: "1px solid rgba(16,185,129,0.15)" }}>
        <div className="absolute top-0 left-0 right-0 h-px" style={{ background: "linear-gradient(90deg, transparent, #10b981, #06b6d4, transparent)" }} />
        ...
        <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: 28, height: 28 }}>
          <svg width="28" height="28" viewBox="0 0 28 28" className="absolute">
            <polygon points="14,2 24,8 24,20 14,26 4,20 4,8" fill="rgba(16,185,129,0.1)" stroke="#10b981" strokeWidth="1" opacity="0.8" />
          </svg>
          <BarChart3 style={{ width: 11, height: 11, color: "#10b981", position: "relative", zIndex: 1 }} />
        </div>

        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: "#10b981", textShadow: "0 0 8px rgba(16,185,129,0.5)", fontFamily: "monospace" }}>FLEETSHEET</span>
          <div className="w-px h-3" style={{ background: "rgba(16,185,129,0.3)" }} />
          <input value={sheetName} onChange={e => setSheetName(e.target.value)}
            className="bg-transparent border-0 text-white text-xs font-mono outline-none placeholder:text-slate-700 w-40 transition-all"
            style={{ caretColor: "#10b981" }}
            placeholder="UNTITLED SHEET" />
        </div>

        <div className="flex items-center gap-1.5 ml-1">
          <div className={`w-1.5 h-1.5 rounded-full ${isSaving ? 'animate-pulse' : ''}`}
            style={{ background: isSaving ? "#f59e0b" : lastSaved ? "#10b981" : "#334155", boxShadow: lastSaved && !isSaving ? "0 0 4px #10b981" : "none" }} />
          <span className="text-[9px] tracking-widest font-mono" style={{ color: "rgba(148,163,184,0.4)" }}>
            {isSaving ? 'SYNC' : lastSaved ? 'SAVED' : 'UNSAVED'}
          </span>
        </div>
