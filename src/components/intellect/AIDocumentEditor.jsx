...
export default function AIDocumentEditor({ initialContent, initialTitle, initialFileUrl, orgId, onSaved }) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950/80" style={{ fontFamily: 'inherit' }}>
      ...
      {/* ── Header ── JARVIS style */}
      <div className="flex-shrink-0 relative px-4 py-2.5 flex items-center gap-3 flex-shrink-0"
        ...
        <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: 28, height: 28 }}>
          <svg width="28" height="28" viewBox="0 0 28 28" className="absolute">
            <polygon points="14,2 24,8 24,20 14,26 4,20 4,8" fill="rgba(59,130,246,0.1)" stroke="#3b82f6" strokeWidth="1" opacity="0.8" />
          </svg>
          <FileText style={{ width: 11, height: 11, color: "#3b82f6", position: "relative", zIndex: 1 }} />
        </div>

        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: "#3b82f6", textShadow: "0 0 8px rgba(59,130,246,0.5)", fontFamily: "monospace" }}>FLEETDOCS</span>
          <div className="w-px h-3" style={{ background: "rgba(59,130,246,0.3)" }} />
          <input
            type="text"
            value={documentTitle}
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button onClick={generateAI} disabled={aiLoading}
            ...
            {aiLoading ? <Loader2 className="w-3 h-3 inline mr-1 animate-spin" /> : <Wand2 className="w-3 h-3 inline mr-1" />}
            AI REVIEW
          </button>
        </div>
      </div>
