...
const QUICK_PROMPTS = [
  "Analyser flådens overordnede performance og giv mig en strategisk rapport",
  "Identificer alle kritiske alarmer og anbefal handlinger",
  "Hvad er den samlede CO₂-udledning for alle aktive forsendelser?",
  "Giv mig en compliance-oversigt for alle moduler",
  "Optimer alle aktive ruter og estimer besparelser",
  "Vis mig en risikovurdering af hele operationen",
];
      ...
      <div className={`max-w-[80%] relative ${isUser ? "items-end" : "items-start"} flex flex-col gap-1`}>
        ...
        {!isUser && message.content && (
          <button onClick={handleCopy}
            ...
            {copied ? <><CheckCheck className="w-3 h-3 text-green-400" />Kopieret</> : <><Copy className="w-3 h-3" />Kopiér</>}
          </button>
        )}
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {conversations.length === 0 && (
          <div className="text-center py-8 text-slate-500 text-xs font-mono">Ingen samtaler endnu</div>
        )}
...
export default function HarborSuperAgentChat({ onClose }) {
  ...
  const createNewConversation = async () => {
    const conv = await base44.agents.createConversation({
      ...
      metadata: { name: `Samtale ${new Date().toLocaleTimeString('da-DK', { hour: '2-digit', minute: '2-digit' })}` }
    });
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      toast.error("Besked kunne ikke sendes");
    }
    setIsSending(false);
                  ...
                  <div className="flex flex-col items-center justify-center py-12 gap-6">
                    ...
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-xs text-slate-500 max-w-xs leading-relaxed">
                        H.A.R.B.O.R Intellect er din Super AI med fuld adgang til alle platformens moduler
                      </p>
                    </div>
              ...
              <div className="flex-shrink-0 p-4" style={{ borderTop: "1px solid rgba(6,182,212,0.1)" }}>
                <div className="relative flex items-end gap-3 rounded-2xl p-3"
                  ...
                  <textarea
                    ...
                    placeholder="Kommandér H.A.R.B.O.R Intellect... (Enter for at sende)"
                    disabled={isSending}
                    rows={1}
                ...
                <p className="text-[9px] font-mono text-slate-600 text-center mt-2 tracking-wider">
                  H.A.R.B.O.R har adgang til alle platformens entiteter og moduler
                </p>
              </div>
            </>
          )}
