...
// ────────────────────────────────────────────────────────────────────
const AI_WORKERS = [
  { id: "fleet_analyst",     name: "Fleet Analyst",       emoji: "📊", color: "#06b6d4", specialty: "Vehicle performance, utilization, CO2 metrics" },
  { id: "route_optimizer",   name: "Route Optimizer",     emoji: "🗺️", color: "#8b5cf6", specialty: "Logistics routing, cost reduction, fuel optimization" },
  { id: "risk_engine",       name: "Risk Engine",         emoji: "⚠️", color: "#ef4444", specialty: "Risk scoring, anomaly detection, exception handling" },
  { id: "forecaster",        name: "Demand Forecaster",   emoji: "🔮", color: "#10b981", specialty: "30/60/90-day demand, inventory, capacity planning" },
  { id: "financial_ai",      name: "Financial AI",        emoji: "💰", color: "#f59e0b", specialty: "Cost analysis, ROI, budget optimization, savings" },
  { id: "maintenance_bot",   name: "Maintenance Bot",     emoji: "🔧", color: "#06b6d4", specialty: "Predictive maintenance, failure prediction, scheduling" },
  { id: "compliance_guard",  name: "Compliance Guard",    emoji: "🛡️", color: "#10b981", specialty: "Regulatory compliance, audit trails, certifications" },
  { id: "sustainability_ai", name: "Sustainability AI",   emoji: "🌍", color: "#22c55e", specialty: "Carbon footprint, ESG metrics, green optimization" },
  { id: "customer_intel",    name: "Customer Intel",      emoji: "👥", color: "#8b5cf6", specialty: "Customer analysis, satisfaction, contract performance" },
  { id: "data_miner",        name: "Data Miner",          emoji: "⛏️", color: "#64748b", specialty: "Pattern recognition, historical data, correlations" },
  { id: "driver_coach",      name: "Driver Coach",        emoji: "🏆", color: "#f59e0b", specialty: "Driver performance, safety scores, training needs" },
  { id: "market_scout",      name: "Market Scout",        emoji: "🔍", color: "#06b6d4", specialty: "Market intelligence, competitor analysis, trends" },
  { id: "ops_commander",     name: "Ops Commander",       emoji: "⚡", color: "#ef4444", specialty: "Real-time operations, dispatch, incident response" },
  { id: "document_ai",       name: "Document AI",         emoji: "📄", color: "#8b5cf6", specialty: "CMR, BOL, contracts, automated documentation" },
  { id: "strategy_ai",       name: "Strategy AI",         emoji: "🧠", color: "#a78bfa", specialty: "Strategic planning, competitive positioning, growth" },
  { id: "api_integrator",    name: "API Integrator",      emoji: "🔗", color: "#06b6d4", specialty: "Data integration, AIS, ADS-B, external APIs" },
  { id: "visualizer",        name: "Visualizer",          emoji: "🎨", color: "#f59e0b", specialty: "Charts, dashboards, heatmaps, live infographics" },
  { id: "nlp_engine",        name: "NLP Engine",          emoji: "💬", color: "#8b5cf6", specialty: "Language processing, translation, report generation" },
  { id: "simulation_ai",     name: "Simulation AI",       emoji: "🌐", color: "#10b981", specialty: "Scenario simulation, digital twins, what-if analysis" },
  { id: "security_ai",       name: "Security AI",         emoji: "🔒", color: "#ef4444", specialty: "Access control, anomaly detection, threat analysis" },
];
...
// ── Worker Status Card ────────────────────────────────────────────────────
function WorkerCard({ worker, task, output, onView }) {
  const statusColors = { idle: "#64748b", queued: "#f59e0b", running: "#06b6d4", done: "#10b981", error: "#ef4444" };
  const color = statusColors[worker.status] || "#64748b";

  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      className="p-3 rounded-xl border transition-all"
      style={{ background: "rgba(15,23,42,0.7)", borderColor: `${color}33`, borderLeft: `3px solid ${color}` }}>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs font-bold" style={{ color }}>{worker.emoji} {worker.name}</span>
        <div className="flex items-center gap-1">
          {worker.status === "running" && <Loader2 className="w-3 h-3 animate-spin" style={{ color }} />}
          {worker.status === "done" && <CheckCircle2 className="w-3 h-3 text-green-400" />}
          {worker.status === "error" && <AlertCircle className="w-3 h-3 text-red-400" />}
          {worker.status === "queued" && <Clock className="w-3 h-3 text-yellow-400" />}
        </div>
      </div>
      {task && <p className="text-[10px] text-slate-400 line-clamp-1 mb-1">{task}</p>}
      {output && (
        <div className="text-[10px] text-slate-300 line-clamp-2 mb-1.5">
          {typeof output === "string" ? output : JSON.stringify(output).slice(0, 120)}
        </div>
      )}
      {output && (
        <button onClick={() => onView(worker, output)}
      ...
      <div className="mt-2 h-0.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
        <motion.div className="h-full rounded-full"
          animate={{ width: worker.status === "done" ? "100%" : worker.status === "running" ? "60%" : worker.status === "queued" ? "15%" : "0%" }}
...
function OrchestrationMonitor({ orchestration, onViewOutput }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(5,10,30,0.95)", border: "1px solid rgba(6,182,212,0.2)" }}>
      {/* Header */}
      <div className="p-3 border-b border-slate-700/50">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-bold font-mono uppercase tracking-widest" style={{ color: "#06b6d4" }}>
            ⚡ Orchestration Live
          </span>
          <div className="flex gap-3 text-[10px] font-mono">
            <span style={{ color: "#10b981" }}>{done} done</span>
            {running > 0 && <span style={{ color: "#06b6d4" }} className="animate-pulse">{running} running</span>}
        ...
        <div className="flex justify-between text-[10px] font-mono mt-1">
          <span style={{ color: "rgba(6,182,212,0.5)" }}>{done}/{total} workers</span>
          <span style={{ color: "rgba(6,182,212,0.5)" }}>{progress}%</span>
        </div>
      </div>
      ...
      <div className="p-3 grid grid-cols-2 gap-2 max-h-80 overflow-y-auto">
        {orchestration.workers.map(w => (
          <WorkerCard
            key={w.id}
            worker={w}
            task={orchestration.tasks.find(t => t.workerId === w.workerId)?.prompt}
            output={orchestration.outputs[w.id]}
            onView={onViewOutput}
          />
        ))}
      </div>
      ...
      <AnimatePresence>
        {showWorkerPool && (
          <motion.div
            ...
            <div className="p-4">
              <p className="text-[10px] font-mono uppercase tracking-widest mb-3" style={{ color: "#64748b" }}>20 Specialized AI Workers</p>
            ...
            <>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 gap-6">
                    ...
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-xs text-slate-500 max-w-xs leading-relaxed">
                        H.A.R.B.O.R Intellect has 20 specialized AI workers standing by. Use ORCHESTRATE to run multiple AI tasks in parallel, or chat directly.
                      </p>
                    </div>
              ...
              <div className="flex-shrink-0 p-4" style={{ borderTop: "1px solid rgba(6,182,212,0.1)" }}>
                ...
                <p className="text-[9px] font-mono text-slate-600 text-center mt-2 tracking-wider">
                  20 AI Workers • Parallel execution • Real-time orchestration • Hologram control
                </p>
              </div>
            </>
          )}
