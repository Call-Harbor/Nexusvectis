...
export default function FuturisticGlobe({ vehicles = [], routes = [], resources = [], digitalTwins = [], onSelectVehicle, onSelectResource }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Update state with combined holograms
      setVisibleRoutes(finalHolograms.filter(h => h.type === 'route' || h.type === 'combined'));
      setVisibleResources(finalHolograms.filter(h => h.type === 'resource'));
      setVisibleVehicles(finalHolograms.filter(h => h.type === 'vehicle'));
