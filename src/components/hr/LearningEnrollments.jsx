...
const ENROLLMENT_STATUS = {
  requested:   { label: "Anmodet",        cls: "bg-slate-500/20 text-slate-400 border-slate-500/30", icon: Clock },
  approved:    { label: "Godkendt",       cls: "bg-blue-500/20 text-blue-400 border-blue-500/30",    icon: CheckCircle2 },
  in_progress: { label: "I gang",         cls: "bg-amber-500/20 text-amber-400 border-amber-500/30", icon: PlayCircle },
  completed:   { label: "Gennemført",     cls: "bg-emerald-500/20 text-emerald-400 border-emerald-500/30", icon: CheckCircle2 },
  cancelled:   { label: "Annulleret",     cls: "bg-rose-500/20 text-rose-400 border-rose-500/30",    icon: XCircle },
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "Totale tilmeldinger", value: stats.total,       color: "text-white" },
          { label: "Gennemført",          value: stats.completed,   color: "text-emerald-400" },
          { label: "I gang",              value: stats.in_progress, color: "text-amber-400" },
          { label: "Afventer godkendt",   value: stats.requested,   color: "text-blue-400" },
        ].map(s => (
      ...
      <div className="flex gap-3 items-center">
        ...
        <span className="text-xs text-slate-500">{filtered.length} tilmeldinger</span>
      </div>
              ...
              <button className="w-full px-4 py-3 flex items-center justify-between hover:bg-slate-800/20 transition-colors"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-slate-500">{(course.enrollments || []).length} tilmeldt{course.capacity ? ` / ${course.capacity} pladser` : ""}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-emerald-400 hover:text-emerald-300 px-2"
                    ...
                    <Plus className="w-3 h-3 mr-1" /> Tilmeld
                  </Button>
              ...
              {showEnrollForm === course.id && (
                <div className="px-4 pb-3 border-t border-slate-800/60 pt-3 flex gap-2">
                  <Select value={enrollEmpId} onValueChange={setEnrollEmpId}>
                    <SelectTrigger className="bg-slate-800/60 border-slate-700 text-white h-8 text-xs flex-1"><SelectValue placeholder="Vælg medarbejder..." /></SelectTrigger>
                    <SelectContent className="bg-slate-900 border-slate-700 text-white">
                      {employees.map(e => <SelectItem key={e.id} value={e.id} className="text-xs">{e.first_name} {e.last_name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <Button size="sm" onClick={() => addEnrollment(course)} disabled={!enrollEmpId || saving} className="h-8 text-xs bg-emerald-600 hover:bg-emerald-500">Tilmeld</Button>
                  <Button size="sm" variant="ghost" onClick={() => setShowEnrollForm(null)} className="h-8 text-xs text-slate-400">Annuller</Button>
                </div>
              )}
              ...
              {isExp && (
                <div className="border-t border-slate-800/60">
                  {courseEnrollments.length === 0 ? (
                    <p className="text-xs text-slate-500 text-center py-4">Ingen tilmeldinger</p>
                  ) : (
                        ...
                        return (
                          <div key={i} className="px-4 py-3 flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {enr.completion_date && <p className="text-[10px] text-emerald-400 mt-0.5">Gennemført {enr.completion_date}</p>}
                            </div>
